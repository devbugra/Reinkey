...
/**
 * Paranın yolu: ajan hesabı → ödeme kanalı → satıcı.
...
import { big, clock, duration, int, ratio, shortAddr, usdc } from "@/lib/format";
...
export function Flow({
  ...
  return (
    <section aria-label={t("aria")} className="grid gap-2 xl:grid-cols-[1fr_auto_1fr_auto_1fr] xl:items-stretch">
      <Card
        step={1}
          ...
          <>
            {rejection && (
              <div className="flex items-start gap-2.5 rounded-md border border-danger/30 bg-danger-bg px-3 py-2.5" role="alert">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-fg">{t("blocked", { reason: describeCode(rejection.code) })}</p>
            ...
            <dl className="grid grid-cols-2 gap-x-4 gap-y-3">
              ...
              <Stat label={t("account")} value={<span className="font-mono text-xs">{shortAddr(account.address, 5, 5)}</span>} />
              <Stat label={t("usdc")} value={usdc(account.balance)} />
              <Stat label={t("xlm")} value={account.balanceXlm ? usdc(account.balanceXlm, 2) : "—"} />
            </dl>
            <div>
              <p className="text-[11px] text-fg-subtle">{t("allowed")}</p>
              <ul className="mt-1.5 flex flex-wrap gap-1.5 text-[11px]">
                ...
                <li className="rounded-sm border border-line bg-bg-alt px-2 py-0.5 text-fg-muted">
                  {t("payeeCount", { count: account.policy.payees.length })}
                </li>
              </ul>
            </div>
          </>
        ) : (
          <p className="py-6 text-center text-sm text-fg-subtle">{t("readingAccount")}</p>
        )}
      </Card>

      <Connector label={t("lockLabel")} sub={t("lockSub")} chainKey={current ? `${current.id}-${current.deposit}` : undefined} />

      <Card
        step={2}
        icon={<Lock className="size-5" aria-hidden="true" />}
        title={t("channelTitle")}
        subtitle={t("channelSub")}
        badge={
          current ? (
            <span
              className={cn(
            ...
            >
              #{current.id} · {!current.open ? t("closed") : exhausted ? t("exhausted") : t("open")}
            </span>
          ) : null
        }
      >
        {current ? (
            ...
            <div>
              ...
              <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-fg-muted">
                <li className="flex items-center gap-1.5">
                  <span className="size-2 rounded-full bg-brand-lavender" aria-hidden="true" /> {t("legendClaimed")}
                </li>
                <li className="flex items-center gap-1.5">
                  <span className="size-2 rounded-full bg-brand-sky" aria-hidden="true" /> {t("legendPending")}
                </li>
                <li className="flex items-center gap-1.5">
                  <span className="size-2 rounded-full bg-surface-3" aria-hidden="true" /> {t("legendLeft")}
                </li>
              </ul>
            </div>
            <dl className="grid grid-cols-3 gap-x-4 gap-y-3">
              <Stat label={t("deposit")} value={usdc(current.deposit)} />
              <Stat label={t("paid")} value={usdc(current.accepted)} />
              <Stat label={current.refunded !== null ? t("refunded") : t("left")} value={usdc(current.refunded ?? remaining)} />
            </dl>
            <p className="tabular flex flex-wrap items-center gap-2 text-xs text-fg-muted">
              <span className="font-semibold text-fg">{t("vouchers", { count: current.vouchers })}</span>
              {stream && !stream.ended && (
                <span className="inline-flex items-center gap-1.5 text-accent">
                  <span className="pulse-dot size-1.5 rounded-full bg-accent" aria-hidden="true" />
                  {stream.unit === "second" ? t("streamPrice") : t("streamResponse")}
                </span>
              )}
          ...
          <p className="py-6 text-center text-sm text-fg-subtle">{t("noChannel")}</p>
        )}
      </Card>
      ...
      <Card
        ...
        title={t("sellerTitle")}
        subtitle={t("sellerSub")}
      >
        <dl className="grid grid-cols-2 gap-x-4 gap-y-3">
          <Stat label={t("claimed")} value={<span className="text-base font-semibold">{usdc(totalClaimed)} USDC</span>} />
          <Stat label={t("pendingClaim")} value={`${usdc(totalPending)} USDC`} />
          <Stat label={t("dataSold")} value={duration(dataSeconds)} />
          <Stat label={t("claimCount")} value={int(claims.length)} />
        </dl>
        {lastClaim ? (
          <div className="rounded-md border border-line bg-bg-alt px-3 py-2.5">
            <p className="text-[11px] text-fg-subtle">{t("lastClaim")}</p>
            <p className="tabular mt-1 flex flex-wrap items-center gap-2 text-sm">
              <span className="font-semibold text-accent">{t("paymentsN", { count: lastClaim.vouchersCovered })}</span>
              <span className="text-fg-subtle">→</span>
              <span className="font-semibold">{t("oneTx")}</span>
              <span className="text-xs text-fg-muted">· {usdc(lastClaim.amount)} USDC</span>
              <TxLink hash={lastClaim.tx} className="ml-auto" />
            </p>
          </div>
        ) : (
          <p className="text-xs text-fg-subtle">{t("autoClaim")}</p>
        )}
        {current && pending > 0n && (
          <button
            type="button"
          ...
          >
            ...
            {t("claimNow", { amount: usdc(pending) })}
          </button>
        )}
      </Card>
    </section>
