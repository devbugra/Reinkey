...
export const metadata = docsMeta(
  "/docs/float",
  "Float: credit pool",
  "A lending pool for agent accounts: capital handed to a Reinkey account can't leave its policy, so it needs no collateral. Deposit and withdraw from the console; the HTTP API is read-only.",
);
...
> **Testnet, unaudited.** The pool contract is deployed and tested. **Deposit and withdraw are live in the [console](/docs/console)**, signed by your own wallet and sent straight to Stellar. **The HTTP API is read-only** — every `/float` endpoint reads by simulation and never submits a transaction. Opening a credit line and liquidating are contract calls with no UI, and `open_line` is the pool admin's decision: there is no permissionless borrowing.

## How it works

1. **Deposit.** An investor deposits USDC and receives shares. `share price = (idle USDC + outstanding debt + pool XLM × price) / total shares`. This is a wallet-signed `deposit(from, amount)` in the console's **Float** view; `withdraw(from, shares)` burns them back, limited to the pool's idle USDC.
...
The HTTP surface is **read-only**: these endpoints read the contract by simulation and never submit a transaction. Money only moves when a wallet signs, in the [console](/docs/console) or against the contract directly.
...
Everything is read by simulation; these endpoints never submit a transaction. Reason codes `40`–`49` come from this contract — see [Reason codes](/docs/reason-codes).

## Contract interface

| Call | Who | Effect |
|---|---|---|
| `deposit(from, amount)` → shares | anyone | Deposit USDC, mint shares at the current share price. Wired to a wallet in the console. |
| `withdraw(from, shares)` → USDC | share holder | Burn shares for USDC, limited to the idle balance. Wired to a wallet in the console. |
