...
**Durum:** v4, yön kilitlendi (fintech + kripto borsaları), geliştirmede
**Tarih:** 19 Eylül 2026
**Etkinlik:** Stellar Pro Hackathon, İstanbul, 19–20 Eylül, Genesis track
**Ağ:** Stellar testnet (Soroban)
**Önceki sürümler:** `docs/archive/proje-tanimi.v1.md` (ajan-yerli borsa), `.v2.md` (Mandate + intent borsası), `.v3.md` (sayaçlı x402, genel)
...
**Reinkey, kripto borsaları ve yapay zekâ ajanları için Stellar üzerinde x402 altyapısıdır.** Kendi borsamızı kurmuyoruz; borsaların ve ajanların kullanacağı rayı kuruyoruz.

Fintech dilinde iki ürün:
- **Satıcı tarafı: kullanım bazlı tahsilat.** Borsa (ya da herhangi bir API) verisini çağrı başına ya da saniye başına satar; para anında gelir, fatura ve vade yok.
- **Alıcı tarafı: ajanlar için harcama ve işlem yönetimi.** Ajanın hesabına tavan, izinli satıcılar ve izinli işlem çiftleri yazılır; sınırı zincir uygular. "AI ajanları için kurumsal kart" + "sınırı zincirde olan API anahtarı".

**Borsalar için üç katman:**
...
**Tek cümle (sunum):** *Borsalar verisini saniye başı satsın, ajanlar zincirin koyduğu sınırla işlem yapsın; ikisi de Stellar'da, x402 ile.*

**Yol haritası (sunumda tek slayt):** anchor'lar üzerinden TL'ye çıkış (x402 ile tetiklenen ödeme), çağrı başına KYC ve yaptırım taraması, kredi havuzu (§3.5).
...
Zincir (reinkey-account)          Zincir (channel)            Zincir dışı
 ...
 8 PAIR_NOT_ALLOWED               27 INVALID_ARGUMENT
 9 SLIPPAGE_UNBOUNDED
```

### 3.7 Katman 2: DEX kuralı (`reinkey-account`)

Politikaya işlem alanları eklenir:

```rust
dex_router: Option<Address>,   // izinli DEX router'ı (Soroswap)
pairs: Vec<(Address, Address)>,// izinli (satılan, alınan) varlık çiftleri
```

`__check_auth` yeni bağlam: `router.swap_exact_tokens_for_tokens(amount_in, amount_out_min, path, to, deadline)` (kesin imza Soroswap testnet sürümünden doğrulanır).
- `to == self` olmalı (satın alınan varlık yalnızca ajanın kendi hesabına gelir) → aksi `PAYEE_NOT_ALLOWED`
- `(path[0], path[last]) ∈ pairs` → aksi `PAIR_NOT_ALLOWED` (yeni kod 8)
- Satılan varlık USDC ise `amount_in ≤ per_tx_cap` ve günlük tavana sayılır; iç içe gelen `transfer` bağlamı çift sayılmaz
- `amount_out_min > 0` zorunlu (kayma koruması olmayan swap reddedilir) → `SLIPPAGE_UNBOUNDED` (yeni kod 9)

Testnette likidite yoksa USDC/XLM havuzuna kurulum betiğiyle likidite eklenir. Kendi AMM'imizi yazmıyoruz.
...
```
GET  /demo/book?pair=USDC_XLM   Çağrı başına 0,0005 USDC   (unit: request)   emir defteri anlık görüntüsü
GET  /demo/ticker/stream        Saniye başına 0,0001 USDC  (unit: second, SSE) canlı fiyat akışı  ← katman 1, demonun yıldızı
POST /demo/chat                 Token başına 0,00002 USDC  (unit: token, SSE)  ikincil örnek
```
...
dex.swapped      { account, sold, soldAsset, bought, boughtAsset, tx }   (ajan bildirir, backend tx'i doğrular)
```
...
Sahne: solda panel (canlı), sağda ajanın terminali. Anlatı: *bir işlem ajanı, bir borsanın verisini satın alıp o borsada işlem yapıyor.*

1. **Yetki.** Sahip bir Reinkey Account kurar: günde 5 USDC, işlem başına 1 USDC, yalnızca USDC↔XLM, yalnızca demo borsa ve DEX. Ajan hiçbir yere kayıt olmamış, API anahtarı almamış.
2. **Kanal.** Ajan 1 USDC'lik kanal açar; tek işlem hash'i görünür. (Katman 3 hazırsa: ajanın elinde XLM vardır, açılışta USDC'ye çevrilir.)
3. **Saniye başı veri.** Ajan borsanın canlı fiyat akışına bağlanır. Panelde sayaç saniye saniye akar: *90 saniye, 90 ödeme, 0 zincir işlemi.*
4. **Karşılaştırma.** Panelde canlı hesap: aynı veri `exact` ile 90 işlem ve ~7,5 dakika.
5. **İşlem.** Ajan fiyat sinyalini görür, DEX'te 0,5 USDC'lik XLM alır. İşlem zincirde kapanır.
6. **Zincirden red 1.** Ajan 2 USDC'lik işlem dener: `PER_TX_CAP_EXCEEDED`, başarısız tx hash'iyle.
7. **Zincirden red 2.** Ele geçirilmiş ajan senaryosu: parayı kendi cüzdanına göndermeye çalışır: `PAYEE_NOT_ALLOWED`. *"Anahtar ajanda, dizgin sizde."*
8. **Kesinti.** Depozito biter, fiyat akışı kesilir: `CHANNEL_EXHAUSTED`.
9. **Tahsilat.** Borsa tek `claim` işlemiyle tüm veri gelirini tahsil eder. Toplam: birkaç zincir işlemi, yüzlerce ödeme.

**Kapanış cümlesi:** Bu bir video değil, şu anda canlı bir URL. Herhangi bir borsa veri akışına tek satırla ekler; jürinin kendi ajanı da MCP üzerinden bağlanabilir.

**Sunumda söylenmeyecekler:** teknik zorluklar, yetişmeyen özellikler, yol haritası detayı. Soru gelirse cevaplanır.

**Hazır cevaplar:**
- *"Kendi borsanızı mı yapıyorsunuz?"* Hayır. Borsaların verisini satacağı ve ajanların sınırlı işlem yapacağı rayı yapıyoruz. DEX tarafında Soroswap'ı kullanıyoruz.
- *"Merkezi borsada bu sınır nasıl uygulanır?"* Merkezi borsaya sattığımız ürün veri ve tahsilat; zincirde uygulanan işlem sınırı DEX tarafında geçerli.
- *"Neden Stellar?"* Özel hesap auth'u (`__check_auth`), protokolün içindeki DEX, ücret sponsorluğu ve sentin kesri işlem maliyeti.
...
- [x] Konumlandırma: kripto borsaları + ajanlar için x402 altyapısı (fintech), üç katman (§0)
